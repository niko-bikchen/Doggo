...
const DogwalkerDetailedCard = () => {
    ...
    if(id!=null && dogWalker.id==null){
        Client.query({query:QUERY,variables:{id}}).then((({data})=>setDogwalker(data.dogwalkers[0])))
    }
    ...
    return (
        <PageBase footerParams={{ theme: 'dark' }}>
            <Grid spacing={3} style={{padding:"20px"}} direction={isMd?"row-reverse":"row"} container>
                            ...
                            <div style={
                                {
                                    ...
                                    width:AVATAR_SIZE+"px",borderRadius:"50%",
                                    background:"white",

                                }}>
                                {avatar_url===""?<Skeleton  variant={"circle"} width={AVATAR_SIZE} height={AVATAR_SIZE}/>
                                    :<Avatar style={{width:AVATAR_SIZE+"px",height:AVATAR_SIZE+"px"}} src={avatar_url}/>}
                            </div>
                        </Grid>
                        <Grid xs={12} item>
                            ...
                            {_.rest(_.pairs(schedule)).map(([k,v],i)=>{
                                return (
                                    <div style={{padding:"10px",backgroundColor:"white",margin:"20px 0"}}>
                ...
                <Grid xs={12} md={8}  item>
                    <Grid spacing={3} container>
                        <Grid item style={{backgroundColor:"white"}} xs={12}>
                            <Typography style={{padding:"15px", fontWeight:"bold"}} variant={"h5"}>{name}</Typography>
                            <Divider/>
                        ...
                        <Grid item style={{paddingLeft:"0px",paddingRight:"0px"}} xs={12}>
