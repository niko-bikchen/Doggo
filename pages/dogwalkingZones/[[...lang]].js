...
import StyledTreeView, {
  StyledTreeItem,
} from "../../components/StyledTreeView";
...
export async function getStaticProps({ params }) {
  ...
  const text = ua
    ? dogwalkingZonesPageText.content_ua
    : dogwalkingZonesPageText.content_ru;
  return { props: { dogWalkingZones, text } };
}

const LocationsToTree = React.memo(
  ({ locations, onAddressClick }) => {
    let count = 0;

    return (
      <StyledTreeView>
        {_.values(
          _.mapObject(locations, (value, key) => {
            return (
              <StyledTreeItem
                nodeId={key}
                key={key}
...
const DogwalkingZones = ({ dogWalkingZones, text }) => {
  ...
  const [currLocation, setCurrLocation] = useState({
    lat: dogWalkingZones[0].lat,
    lng: dogWalkingZones[0].lng,
  });
  ...
  return (
    <PageBase footerParams={{ theme: "dark" }}>
      <NextSeo
        canonical="https://doggo.co.ua/dogwalkingZones"
        title="Doggo | Місця для вигулу"
        description="Багато жителів Києва мають домашніх улюбленців, яких потрібно регулярно вигулювати для підтримання здорового і активного образу життя. В місті можна знайти спеціальні зони для вигулу собак. За офіційними даними їх налічується 130."
      />
